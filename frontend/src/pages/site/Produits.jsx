...
export default function Produits() {
  ...
  useEffect(() => {
    ...
    params.set("limit", String(MAX_PER_TYPE));
    api
  ...
  // 4 produits max par type (par famille de la barre latérale).
  const page = shown.slice(0, MAX_PER_TYPE);
          ...
          <div>
            ...
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
              <span className="text-sm text-ash">
                {shown.length > MAX_PER_TYPE ? `${MAX_PER_TYPE} produits affichés sur ${shown.length}` : `${shown.length} produit(s)`}
              </span>
