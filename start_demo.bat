@echo off
setlocal
title KOUDI WOOD - Demo Launcher
cd /d "%~dp0"

echo ============================================================
echo   KOUDI WOOD - Vente & Gestion de Stock du Bois (demo presentation)
...
echo       (fenetre separee : "KOUDI WOOD backend 8001")
start "KOUDI WOOD backend 8001" cmd /k "cd /d ""%~dp0backend"" & python manage.py runserver 127.0.0.1:8001 --noreload"
...
echo.
echo KOUDI WOOD est disponible sur  http://127.0.0.1:8001
echo.
echo - Le backend (API + interface) tourne dans la fenetre
echo   "KOUDI WOOD backend 8001" : fermez-la pour arreter.
